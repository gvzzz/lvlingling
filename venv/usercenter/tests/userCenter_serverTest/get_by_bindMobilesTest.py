...
#第0个json入参
def dev_get_by_bindMobiles_Open(i):
    #将lion开关打开
    utils.lion.modifylion(17439, 3, "true", 30)
    time.sleep(1)
    #获取上上级目录
    path_base = os.path.abspath(os.path.join(os.getcwd(), "../.."))
    path = path_base + "/hcbdata/get_by_bindMobiles.json"  # 拼成绝对路径
    url = "http://ucenter.dev-ag.56qq.com"
    openLionStr = base.userCenter_server.get_by_bindMobiles(url,path,i)
    openLionJson = json.loads(openLionStr)
    return openLionJson


def dev_get_by_bindMobiles_Clost(i):
    # 获取上上级目录
    path_base = os.path.abspath(os.path.join(os.getcwd(), "../.."))
    path = path_base + "/hcbdata/get_by_bindMobiles.json"  # 拼成绝对路径
    url = "http://ucenter.dev-ag.56qq.com"
    ...
    time.sleep(1)
    closeLionStr = base.userCenter_server.get_by_bindMobiles(url, path, i)
    closeLionJson = json.loads(closeLionStr)
    return closeLionJson


def jsondif(i):
    result = diff(dev_get_by_bindMobiles_Open(i), dev_get_by_bindMobiles_Clost(i))
...
def writeExcelDaliy(openList,closeList,inputIsSameList,difList):
    ...
    myWorkbook.save(today +"get_by_bindMobiles"+"dev"+ '.xls')
...
if __name__ == '__main__':
    ...
    inputIsSameList= []
    difList = []
    openList.append(dev_get_by_bindMobiles_Open(0))
    closeList.append(dev_get_by_bindMobiles_Clost(0))
    inputIsSameList.append("第0个json入参是随机入参")
