...
def dev_consignor_get_Open(i):
    ...
    openLionStr = base.userCenter_server.consignor_get(url,path,i)
    openLionJson = json.loads(openLionStr)
    return openLionJson


def dev_consignor_get_Clost(i):
    # 获取上上级目录
    path_base = os.path.abspath(os.path.join(os.getcwd(), "../.."))
    path = path_base + "/hcbdata/consignor_get.json"  # 拼成绝对路径
    url = "http://ucenter.dev-ag.56qq.com"
...
def jsondif(i):
    result = diff(dev_consignor_get_Open(i), dev_consignor_get_Clost(i))
...
if __name__ == '__main__':
    ...
    inputIsSameList= []
    difList = []
    openList.append(dev_consignor_get_Open(0))
    closeList.append(dev_consignor_get_Clost(0))
    inputIsSameList.append("第0个json入参是一致性司机")
    difList.append(jsondif(0))
    writeExcelDaliy(openList, closeList,inputIsSameList,difList)

    openList.append(dev_consignor_get_Open(1))
    closeList.append(dev_consignor_get_Clost(1))
    inputIsSameList.append("第1个json入参是非一致性司机")
    difList.append(jsondif(1))
    writeExcelDaliy(openList, closeList, inputIsSameList, difList)

    openList.append(dev_consignor_get_Open(2))
    closeList.append(dev_consignor_get_Clost(2))
    inputIsSameList.append("第2个json入参是一致性货主")
    difList.append(jsondif(2))
    writeExcelDaliy(openList, closeList, inputIsSameList, difList)

    openList.append(dev_consignor_get_Open(3))
    closeList.append(dev_consignor_get_Clost(3))
