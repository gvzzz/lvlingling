...
#第0个json入参
def dev_get_with_virtual_info_Open(i):
    #将lion开关打开
    utils.lion.modifylion(17439, 3, "true", 30)
    time.sleep(5)
    #获取上上级目录
    path_base = os.path.abspath(os.path.join(os.getcwd(), "../.."))
    path = path_base + "/hcbdata/get_with_virtual_info.json"  # 拼成绝对路径
    url = "http://ucenter.dev-ag.56qq.com"
    openLionStr = base.userCenter_server.get_with_virtual_info(url,path,i)
    openLionJson = json.loads(openLionStr)
    return openLionJson


def dev_get_with_virtual_info_Clost(i):
    # 获取上上级目录
    path_base = os.path.abspath(os.path.join(os.getcwd(), "../.."))
    path = path_base + "/hcbdata/get_with_virtual_info.json"  # 拼成绝对路径
    url = "http://ucenter.dev-ag.56qq.com"
    ...
    time.sleep(5)
    closeLionStr = base.userCenter_server.get_with_virtual_info(url, path, i)
    closeLionJson = json.loads(closeLionStr)
    return closeLionJson


def jsondif(i):
    result = diff(dev_get_with_virtual_info_Open(i), dev_get_with_virtual_info_Clost(i))
...
def writeExcelDaliy(openList,closeList,inputIsSameList,difList):
    ...
    myWorkbook.save(today +"get_with_virtual_info"+"dev"+ '.xls')
...
if __name__ == '__main__':
    ...
    inputIsSameList= []
    difList = []
    openList.append(dev_get_with_virtual_info_Open(0))
    closeList.append(dev_get_with_virtual_info_Clost(0))
    inputIsSameList.append("第0个json入参是一致性司机")
    difList.append(jsondif(0))
    writeExcelDaliy(openList, closeList,inputIsSameList,difList)

    openList.append(dev_get_with_virtual_info_Open(1))
    closeList.append(dev_get_with_virtual_info_Clost(1))
    inputIsSameList.append("第1个json入参是非一致性司机")
    difList.append(jsondif(1))
    writeExcelDaliy(openList, closeList, inputIsSameList, difList)

    openList.append(dev_get_with_virtual_info_Open(2))
    closeList.append(dev_get_with_virtual_info_Clost(2))
    inputIsSameList.append("第2个json入参是一致性货主")
    difList.append(jsondif(2))
    writeExcelDaliy(openList, closeList, inputIsSameList, difList)

    openList.append(dev_get_with_virtual_info_Open(3))
    closeList.append(dev_get_with_virtual_info_Clost(3))
